...
from myapp.views import model, calibrated_model
import numpy as np

class Command(BaseCommand):
    help = 'Test the calibrated model with different inputs'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"{'Case Name':<15} {'Original Prediction':<20} {'Original Probability':<25} {'Calibrated Prediction':<25} {'Calibrated Probability':<25}")
        ...
        for case in test_cases:
            ...
            # Print results
            self.stdout.write(f"{case['name']:<15} {orig_pred} ({orig_pred_str:<15}) {orig_proba_percent[0]}% / {orig_proba_percent[1]}%{' ':10} {calib_pred} ({calib_pred_str:<15}) {calib_proba_percent[0]}% / {calib_proba_percent[1]}%") 